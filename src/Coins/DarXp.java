        ...
        if (cmd.getName().equalsIgnoreCase("gratis")) {
            if (!p.hasPermission("arkuz.gratis")) {
                ...
                if (target == null || !(target instanceof Player)) {
                    sender.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Esse Player N\u00e3o Est\u00e1 Conectado Ao Servidor");
                    return true;
                }
                if (isNumeric(args[1])) {
                    final int coins = Integer.parseInt(args[1]);
                    XpM.addMoney(target, coins);
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Voc\u00ea Deu Ao Jogador §c" + target.getName() + " §4" + coins + " §7MP");
                    target.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Voc\u00ea Recebeu Do Jogador §c" + p.getName() + " §4" + coins + " §7MP");
                }
            }
        }
        return false;
