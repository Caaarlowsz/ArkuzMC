...
public class InvVip implements Listener
{
    ...
    public static void inventory(final Player p) {
        ...
        menu.setItem(3, criarItemMenu(Material.GOLD_BLOCK, "§aVip", new String[] { " ", "              §b§lVANTAGENS", "§7» §fVaga reservada no servidor.", "§a» §fTag §aVip §fpara ser diferenciado.", "§a» §fAcesso ao §6/FAKE §fe §6/SKIN", "§a» §fMelhor suporte", "§a» §fFalar colorido", "§a» §fAcesso ao §a/FLY", "§a» §aAcesso \u00e0 TODAS classes do servidor.", " " }));
    ...
    public void clickarloja(final InventoryClickEvent e) {
        ...
        if (e.getInventory().getTitle().equals("§aMenu de Lojas") && e.getCurrentItem() != null && e.getCurrentItem().getTypeId() != 0) {
            e.setCancelled(true);
    ...
    public void clickarlojavips(final InventoryClickEvent e) {
        final Player p = (Player)e.getWhoClicked();
        if (e.getInventory().getTitle().equals("§aMenu de §6§lVIPS") && e.getCurrentItem() != null && e.getCurrentItem().getTypeId() != 0) {
            e.setCancelled(true);
