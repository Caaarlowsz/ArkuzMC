...
public class InvLoja implements Listener, CommandExecutor
{
    ...
    public void onPlayerCLickInventry(final InventoryClickEvent e) {
        ...
        if (e.getInventory().getTitle().equalsIgnoreCase("§8{ §cLoja §8}") && e.getCurrentItem() != null && e.getCurrentItem().getTypeId() != 0) {
            if (e.getSlot() == 14) {
                e.setCancelled(true);
                p.closeInventory();
                p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Entre Em Nossa Loja: §c" + Main.loja);
                return;
            }
            if (e.getSlot() == 12) {
                e.setCancelled(true);
