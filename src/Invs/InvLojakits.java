...
public class InvLojakits implements Listener, CommandExecutor
{
    ...
    public InvLojakits(final Main main) {
    }
        ...
        if (e.getCurrentItem() != null && e.getCurrentItem().getItemMeta() != null) {
            ...
            if (e.getCurrentItem().isSimilar(InvLojakits.ninja)) {
                if (XpM.getPlayerMoney(p) >= 4000) {
                    ...
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Kit §c§nAVATAR§7 Adquirido Com Sucesso");
                    ScoreBoarding.setScoreBoard(p);
                    e.setCancelled(true);
                    p.closeInventory();
                }
                else if (XpM.getPlayerMoney(p) < 4000) {
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Voc\u00ea N\u00e3o Tem XP o Suficiente Para Esta Compra");
                    e.setCancelled(true);
                    p.closeInventory();
                }
            }
            if (e.getCurrentItem().isSimilar(InvLojakits.viking)) {
                if (XpM.getPlayerMoney(p) >= 15000) {
                    ...
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Kit §c§nSTOMPER§7 Adquirido Com Sucesso");
                    ScoreBoarding.setScoreBoard(p);
                    e.setCancelled(true);
                    p.closeInventory();
                }
            ...
            if (e.getCurrentItem().isSimilar(InvLojakits.stomper)) {
                if (XpM.getPlayerMoney(p) >= 1000) {
                    ...
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Kit §c§nTHOR§7 Adquirido Com Sucesso");
                    e.setCancelled(true);
                    p.closeInventory();
                    ScoreBoarding.setScoreBoard(p);
                }
            ...
            if (e.getCurrentItem().isSimilar(InvLojakits.viper)) {
                if (XpM.getPlayerMoney(p) >= 5000) {
                    ...
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Kit §c§nSWORDS§7 Adquirido Com Sucesso");
                    e.setCancelled(true);
                    p.closeInventory();
                    ScoreBoarding.setScoreBoard(p);
                }
            ...
            if (e.getCurrentItem().isSimilar(InvLojakits.snail)) {
                if (XpM.getPlayerMoney(p) >= 4000) {
                    ...
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Kit §c§nFORCEFIELD§7 Adquirido");
                    e.setCancelled(true);
                    p.closeInventory();
                    ScoreBoarding.setScoreBoard(p);
                }
