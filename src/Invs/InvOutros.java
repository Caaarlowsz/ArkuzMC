...
public class InvOutros implements Listener, CommandExecutor
{
    public static void guiKits(final Player p) {
        ...
        descpyro1.add("§9\u27bc Kills: §7" + Main.plugin.getConfig().getInt("status." + p.getName().toLowerCase() + ".kills"));
        descpyro1.add("§4\u27bc Mortes: §7" + Main.plugin.getConfig().getInt("status." + p.getName().toLowerCase() + ".mortes"));
    ...
    public void onPlayerCLickInventry(final InventoryClickEvent e) {
        ...
        if (e.getInventory().getTitle().equalsIgnoreCase("§8{ §cOutros §8}") && e.getCurrentItem() != null && e.getCurrentItem().getTypeId() != 0) {
            e.setCancelled(true);
            if (e.getSlot() == 12) {
                e.setCancelled(true);
                p.closeInventory();
