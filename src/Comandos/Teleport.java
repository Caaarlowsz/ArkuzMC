...
public class Teleport implements CommandExecutor, Listener {
	public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("teleport")) {
			...
			if (args.length == 0) {
				p.sendMessage(String.valueOf(ArkuzKitPvP.prefix) + " §4➼ §7Use: /tp {Player}");
				...
				if (p.hasPermission("arkuz.teleport") || p.isOp()) {
					...
					p.teleport(tpLoc);
					p.sendMessage(String.valueOf(ArkuzKitPvP.prefix) + " §4➼ §7Você Foi Teleportado Para: §c"
				...
				if (p.hasPermission("arkuz.teleport") || p.isOp()) {
					...
					if (targetPlayer == null || targetPlayer2 == null) {
						...
						p.sendMessage(String.valueOf(ArkuzKitPvP.prefix) + " §4➼ §c" + targetPlayer.getDisplayName()
