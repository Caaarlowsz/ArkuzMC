	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
		if (cmd.getName().equalsIgnoreCase("broadcast") || cmd.getName().equalsIgnoreCase("bc")) {
			if (sender.hasPermission("arkuz.bc")) {
				if (args.length == 0) {
					sender.sendMessage(String.valueOf(ArkuzPvP.prefix) + " §4➼ §7Use: /aviso {Menssagem}");
					return true;
				}
				String msg = "";
				...
				Bukkit.broadcastMessage(" ");
				Bukkit.broadcastMessage(String.valueOf(ArkuzPvP.prefix) + " §4➼ §7" + msg.replace("&", "§"));
				Bukkit.broadcastMessage(" ");
				Bukkit.broadcastMessage(" ");
				return true;
			} else {
				sender.sendMessage(String.valueOf(ArkuzPvP.prefix) + " §4➼ §7Sem Permissão");
			}
		}
		return false;
	}
