        ...
        if (commandLabel.equalsIgnoreCase("report")) {
            if (args.length >= 2) {
                ...
                if (target != null) {
                    ...
                    p.sendMessage("§6* §7O Report do jogador §6" + target.getName() + " §7Pelo motivo §6" + reportMsg + "§7 Foi enviado a nossa equipe!");
                    ...
                    for (int j = (arrayOfPlayer = Bukkit.getOnlinePlayers()).length, i = 0; i < j; ++i) {
                        ...
                        if (s.hasPermission("arkuz.report")) {
                            ...
                            s.sendMessage("§c");
                            ChatInterativoAPI.Comando(s.getName(), "§c* §7TELEPORTAR-SE AO JOGADOR§c *", "/tp " + target.getName(), "§eTeleporte Rapido. §7(Clique)");
                            s.sendMessage("§c§m--->-----------------------------<---");
                            s.sendMessage("§c");
