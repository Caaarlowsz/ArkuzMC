    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("vis")) {
            if (p.hasPermission("arkuz.vis")) {
                if (!emInvisivel(p)) {
                    addInvisivel(p);
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Modo Invisivel Ativado");
                    ...
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Modo Invisivel Desativado");
                }
            }
            else {
                p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Sem Permissao");
            }
        }
        return false;
    }
