        ...
        if (cmd.getName().equalsIgnoreCase("build")) {
            if (p.hasPermission("arkuz")) {
                if (args.length == 0) {
                    ...
                    if (t == null) {
                        p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Este Player N\u00e3o Est\u00e1 Conectado Ao Servidor");
                        return true;
                    }
                    if (!Build.embuild.contains(t)) {
                        Build.embuild.add(t);
                        p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Voc\u00ea Ativou o Modo Build Para §c" + t.getName());
                    }
                    else {
                        Build.embuild.remove(t);
                        p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Voc\u00ea Desativou o Modo Build Para §c" + t.getName());
                    }
                }
            }
            else {
