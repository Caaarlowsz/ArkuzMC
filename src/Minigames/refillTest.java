...
public class refillTest
{
    ...
    public refillTest(final Player p) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                if (this.seconds > 0) {
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Treino De Refil Iniciando Em: §c" + this.seconds);
                    --this.seconds;
                    return;
                }
                if (this.seconds == 0) {
                    ...
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7O §c§nTreino§7 §c§nDe§7 §c§nRefil§7 Iniciado");
                    ...
                    Methodos.fillLinhaInventory((Inventory)p.getInventory(), r + 9, sopa, sopa, sopa, sopa, sopa, sopa, sopa, sopa, sopa);
                }
                KitAPI.refillTestTime.put(p.getName(), refillTest.this.getTime(p) + 1);
                p.setLevel(refillTest.this.getTime(p));
                if (refillTest.this.hasRefiled(p)) {
                    p.setExp(0.0f);
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Voc\u00ea Terminou Seu Refil Em: §c" + Stringtimer.timerChecker(KitAPI.refillTestTime.get(p.getName())) + " §7Segundos");
                    p.getInventory().clear();
                    refillTest.setitem(p, Material.BOOK, "§4\u27bc §cWarps", 1, Enchantment.DAMAGE_ALL, 0, false);
                    refillTest.setitem(p, Material.BREWING_STAND_ITEM, "§4\u27bc §cLoja", 2, Enchantment.DAMAGE_ALL, 0, false);
                ...
                if (this.seconds >= 60) {
                    p.sendMessage(String.valueOf(Main.prefix) + " §4\u27bc §7Voc\u00ea Demorou Demais, Tente Mais Uma Vez !");
                    this.cancel();
                }
            }
